...
function loadPhotopea() {
    ...
    gradioApp().getElementById("photopeaIframeContainer").appendChild(iframe);
}
...
function onPhotopeaLoaded(e) {
    ...
    // use the most.
    createSendToPhotopeaButton("image_buttons_txt2img", window.txt2img_gallery[0]);
    createSendToPhotopeaButton("image_buttons_img2img", window.img2img_gallery[0]);
    createSendToPhotopeaButton("image_buttons_extras", window.extras_gallery[0]);
}
...
function goToImg2ImgInpaintUpload(onFinished) {
    ...
    waitForWebUiUpdate(img2imgdiv).then(() => {
        ...
        const inpaintButton = Array.from(allButtons).find(
            (button) => button.textContent === "Inpaint upload ",
        );
        inpaintButton.click();
        onFinished();
    });
...
function sendImageWithMaskSelectionToWebUi() {
    ...
    postMessageToPhotopea(getPhotopeaScriptString(selectionExists)).then((response) => {
        if (response[0] === false) {
...
function setImageOnControlNetInput(controlNetDiv, controlNetModelIndex, file) {
    ...
    waitForWebUiUpdate(controlNetDiv).then(() => {
        ...
        imageInput =
            controlNetDiv.querySelectorAll("input[type='file']")[controlNetModelIndex];
        setImageOnInput(imageInput, file);
    });
}
